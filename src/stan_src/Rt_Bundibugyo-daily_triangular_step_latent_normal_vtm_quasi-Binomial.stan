...
// Daily state-space renewal model, triangular incubation period: latent daily infections with variance-to-mean (VTM)
...
data {
    ...
    // daily-discretised incper and TOST
    int<lower = 1> d_max, d_ost_max;
    vector<lower = 0, upper = 1>[d_max] f_inc;
    vector<lower = 0, upper = 1>[d_ost_max + 1] f_tost;
}
...
transformed data {
    ...
    vector[d_max] f_inc_rev = reverse(f_inc);
    vector[d_ost_max + 1] f_tost_rev = reverse(f_tost);
...
parameters {
    ...
    real<lower = 0> d;                     // variance-to-mean ratio of latent daily infections
...
transformed parameters {
    ...
    for (day in 1 : T) {
        onsets[day] = (day == 1) ? 0.0
            : dot_product(tail(head(infections, day - 1), min(day - 1, d_max)),
                          tail(f_inc_rev, min(day - 1, d_max)));
        if (day <= n_seed) {
            infections[day] = j_seed;                              // latent seed (pre-observation)
        } else {
            int obs_day = day - n_seed;                      // 1 .. 7*(W+3)
            int week = (obs_day - 1) %/% 7 + 1;
            force = dot_product(tail(head(onsets, day), min(day, d_ost_max + 1)),
                                tail(f_tost_rev, min(day, d_ost_max + 1)));
            mean_inf = Rt_infection[week] * force;
            infections[day] = fmax(mean_inf + sqrt(d * fmax(mean_inf, 0.0)) * eps[obs_day], 1e-12);
        }
    }
    for (week in 1 : W + 3) {
...

    {
        ...
        for (week in 1 : W) {
            ...
            if (deaths == 0)
                ...
                target += (lgamma(survived + deaths + 1) - lgamma(deaths + 1) - lgamma(survived + 1) + deaths * log(prob_death) + survived * log1m(prob_death));
        }
    }
}
...
generated quantities {
    ...
    for (week in 1 : W) {
        ...
        log_lik[W + 2 + week] = (deaths == 0) ? c_asc * log1m(prob_death)
            : (survived > 0 ? (lgamma(survived + deaths + 1) - lgamma(deaths + 1) - lgamma(survived + 1) + deaths * log(prob_death) + survived * log1m(prob_death))
