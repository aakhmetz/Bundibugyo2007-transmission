...
// Weekly state-space renewal model: latent weekly infections ~ Normal(R_t*force, sqrt(d*R_t*force)),
// floored above 0 and sampled non-centred (VTM parameterisation, epidemia fixed_vtm = TRUE),
...
parameters {
    ...
    real<lower = 0> d;                     // variance-to-mean ratio of latent infections (fixed_vtm = TRUE)
...
transformed parameters {
    ...
    for (wk in 1 : Tw) {
        ...
        if (wk <= n_seed) {
            ...
            infections[wk] = fmax(mean_inf + sqrt(d * fmax(mean_inf, 0.0)) * eps[ow], 1e-12);
        }
    }
