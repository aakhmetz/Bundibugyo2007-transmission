// Ascertainment-ramp variant: case ascertainment recovers as an exponential ramp toward the
// intervention (time constant tau_rep, weeks) instead of a step; otherwise identical to the
// daily gamma semi-mechanistic uniform quasi-Binomial baseline.
...
data {
    ...
    real<lower = 0> tau_rep;   // ascertainment ramp time-constant (weeks)
}
...
transformed parameters {
    ...
    for (week in 1 : W + 3) {
        ...
        p_asc[week] = t_int > week + 0.5 ? fma(1 - p_check, exp(-(t_int - week - 0.5) / tau_rep), p_check) : 1.0;
    }
}
