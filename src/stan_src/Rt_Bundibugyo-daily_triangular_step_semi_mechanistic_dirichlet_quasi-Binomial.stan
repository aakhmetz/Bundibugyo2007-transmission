...
data {
    ...
    int<lower = 1> d_max, d_ost_max; // maximum number of days in the discretized distributions
    vector<lower = 0, upper = 1>[d_max] f_inc; // discretized incubation period distribution (starts from day 1)
    vector<lower = 0, upper = 1>[d_ost_max + 1] f_tost; // discretized TOST distribution (starts from day 0)
}
...
model {
    ...
    vector[7 * (W + 3)] infections, expected_cases;
    {
        ...
        for (day in 1 : 7 * (W + 3)) {
            int week = (day - 1) %/% 7 + 1;
            expected_cases[day] = (day == 1) ? 0.0 : dot_product(tail(head(infections, day - 1), min(day - 1, d_max)), tail(f_inc_rev, min(day - 1, d_max)));
            cases_asc[day] += (1 - p_asc[week]) * expected_cases[day];
            conv_tost = dot_product(tail(head(cases_asc, day), min(day, d_ost_max + 1)), tail(f_tost_rev, min(day, d_ost_max + 1)));
    ...
    vector[(W + 2) + W] log_lik;           // pointwise log-likelihood for LOO
    {
        ...
        for (week in 1 : W) {
            ...
            log_lik[W + 2 + week] = (deaths == 0) ? c_asc * log1m(prob_death)
                : (survived > 0 ? (lgamma(survived + deaths + 1) - lgamma(deaths + 1) - lgamma(survived + 1) + deaths * log(prob_death) + survived * log1m(prob_death)) : deaths * log(prob_death));
        }
