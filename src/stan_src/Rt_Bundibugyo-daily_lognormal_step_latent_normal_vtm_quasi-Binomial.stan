...
// Daily state-space renewal model, lognormal incubation period: latent daily infections with variance-to-mean (VTM)
...
transformed data {
    ...
    real cv_inc = sd_inc / mean_inc,
        sqr_cv_inc = square(cv_inc),
        sqr_param2_lognormal = log1p(sqr_cv_inc),
        param2_inc = sqrt(sqr_param2_lognormal),
        param1_inc = log(mean_inc) - sqr_param2_lognormal / 2.0;
    vector[d_inc_max] f_rev;
    {
        vector[d_inc_max + 1] res;
        for (k in 1 : d_inc_max + 1)
            res[k] = lognormal_cdf(k - 0.5 | param1_inc, param2_inc);
